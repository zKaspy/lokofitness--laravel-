...
<body>
  ...
  <header>
    ...
    <nav>
      <div class="nav-wrapper indigo darken-2">
        ...
        <a class="breadcrumb" href="{{ route('adminComments') }}">Отзывы</a>
  ...
  <div class="row" style="margin-top: 20px;">
    <form class="col s8" action="{{ route('adminCommentsPost', $id) }}" method="post">
    {{ csrf_field() }}
      <div class="row">
        <div class="input-field col s6">
          <input type="text" class="validate" disabled value="{{ $comment -> user -> name }}">
          <label>Имя отправителя</label>
        </div>
        </div>
        <div class="row">
        <div class="input-field col s6">
          <textarea class="materialize-textarea" disabled>{{ $comment -> text }}</textarea>
    ...
    <div class="col s6">
      <div class="col s2">
          <button class="btn waves-effect waves-light" type="submit" name="save">Принять
          <i class="material-icons right">send</i>
          </button>
      </div>
      <div class="col s2 offset-s4">
        <a class="btn waves-effect waves-light red" href="{{ route('adminCommentsDelete', $id) }}">Удалить
        <i class="material-icons right">send</i>
        </a>
      </div>
      </div>
